...
// Add to SECP256K1.cpp

void Secp256K1::InitHalfMod() {
    if (!half_mod_initialized) {
        // (p+1)/2 for fast halving
        half_mod_p.Set(&P);
        half_mod_p.AddOne();
        half_mod_p.ShiftR(1);
        half_mod_initialized = true;
    }
}

Point Secp256K1::HalvePoint(Point &p) {
    if (!half_mod_initialized) InitHalfMod();
    ...
    // For point halving, we need to find Q such that 2Q = P
    // This means Q = P * (1/2) mod p
    
    // First normalize the point if needed
    Point pn(p);
    if (!pn.isOne()) {
        pn.Reduce();
    }
    
    // Check if y is even or odd to determine which solution we want
    bool yIsEven = pn.y.IsEven();
    
    // Compute x3 = x
    result.x = pn.x;
    
    // Compute y3 = y * (1/2) mod p
    // But we need to handle the two possible solutions
    if (yIsEven) {
        // y is even, simple halving
        result.y.ModMulK1(&pn.y, &half_mod_p);
    } else {
        // y is odd, add p then halve
        result.y.Set(&pn.y);
        result.y.ModAdd(&P);
        result.y.ModMulK1(&half_mod_p);
    }
    
    // Verify the result (optional, remove in production)
    #ifdef DEBUG
    Point doubled = Double(result);
    if (!doubled.equals(pn)) {
        printf("ERROR: Halving verification failed!\n");
    }
    #endif
...
Point Secp256K1::HalvePointFast(Point &p) {
    // Optimized version using the fact that for secp256k1
    // if P = (x, y), then P/2 has the same x-coordinate
    // but y-coordinate needs special handling
    ...
    // Lambda calculation for point halving
    // We need to solve: 2*Q = P
    // This involves finding the correct y-coordinate
    ...
    // Calculate 3x²
    three_x2.ModSquareK1(&p.x);
    temp.Set(&three_x2);
    three_x2.ModAdd(&temp);
    three_x2.ModAdd(&temp);
    
    // Calculate lambda = 3x² / (2y)
    Int two_y;
    two_y.ModAdd(&p.y, &p.y);
    two_y.ModInv();
    lambda.ModMulK1(&three_x2, &two_y);
    
    // Now we need to reverse the doubling formula
    // For doubling: x3 = λ² - 2x
    // For halving: we need to solve for the original x
    
    // This is complex, so we use a different approach:
    // Find s such that (x, s) doubled gives (x, y)
    Int s;
    Int lambda_sq;
    lambda_sq.ModSquareK1(&lambda);
    
    // x_half = (λ² + x) / 2
    temp.ModAdd(&lambda_sq, &p.x);
    if (!half_mod_initialized) InitHalfMod();
    result.x.ModMulK1(&temp, &half_mod_p);
    
    // y_half can be computed from the halving formula
    temp.ModSub(&p.x, &result.x);
    temp.ModMulK1(&lambda);
    result.y.ModSub(&temp, &p.y);
    
    // Adjust y if necessary
    if (!result.y.IsEven() && p.y.IsEven()) {
        result.y.ModAdd(&P);
    }
...
std::vector<Point> Secp256K1::BatchHalve(std::vector<Point> &points) {
    ...
    std::vector<Int*> to_invert;
    std::vector<Int> two_y_values(n);
    
    // Collect all 2y values to invert
    for (size_t i = 0; i < n; i++) {
        if (!points[i].isZero()) {
            two_y_values[i].ModAdd(&points[i].y, &points[i].y);
            to_invert.push_back(&two_y_values[i]);
        }
    }
    
    // Batch inversion
    std::vector<Int> inverses(to_invert.size());
    BatchModInverse(to_invert, inverses);
    
    // Apply halving formula with batch-inverted values
    size_t inv_idx = 0;
    for (size_t i = 0; i < n; i++) {
        if (points[i].isZero()) {
            results[i] = points[i];
            ...
            Int &inv_2y = inverses[inv_idx++];
            
            // Compute lambda = 3x² / (2y)
            Int lambda, three_x2, temp;
            three_x2.ModSquareK1(&p.x);
            temp.Set(&three_x2);
            three_x2.ModAdd(&temp);
            three_x2.ModAdd(&temp);
            lambda.ModMulK1(&three_x2, &inv_2y);
            
            // Compute halved point
            Int lambda_sq;
            lambda_sq.ModSquareK1(&lambda);
            temp.ModAdd(&lambda_sq, &p.x);
            r.x.ModMulK1(&temp, &half_mod_p);
...
void Secp256K1::BatchModInverse(std::vector<Int*> &values, std::vector<Int> &results) {
    size_t n = values.size();
    if (n == 0) return;
    
    results.resize(n);
    if (n == 1) {
        results[0].Set(values[0]);
        results[0].ModInv();
        return;
    }
    ...
    Int inv;
    inv.Set(&products[n-1]);
    inv.ModInv();
    
    // Backward pass: extract individual inverses
    for (size_t i = n - 1; i > 0; i--) {
        results[i].ModMulK1(&inv, &products[i-1]);
        inv.ModMulK1(&inv, values[i]);
    }
    results[0] = inv;
}

uint64_t Secp256K1::HashPoint(const Point &p) {
    // Fast hash using first 8 bytes of x-coordinate
    return p.x.bits64[0] ^ (p.x.bits64[1] << 32);
}

Point Secp256K1::SubtractSmall(Point &p, int value) {
    // Optimized subtraction for small values (like 1)
    // Uses precomputed -G, -2G, -3G, etc.
    
    if (value == 0) return p;
    if (value == 1) {
        // Use the already computed -G
        Point negG = Negation(G);
        return AddDirect(p, negG);
    }
    
    // For larger values, compute -value*G efficiently
    Int scalar;
    scalar.SetInt32(value);
    Point toSubtract = ComputePublicKey(&scalar);
    Point negated = Negation(toSubtract);
    return AddDirect(p, negated);
}
