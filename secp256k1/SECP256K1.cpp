...
Point Secp256K1::Double(Point &p)
{
  ...
  // Use original doubling formula that was known to work
  Int _s;
  Int _p;
  Point r;
  r.z.SetInt32(1);

  _s.ModMulK1(&p1.x, &p1.x); // s = x*x
  _p.ModAdd(&_s, &_s);
  _p.ModAdd(&_s); // p = 3*x*x

  // Compute 2*y for division
  Int _2y;
  _2y.ModAdd(&p1.y, &p1.y); // 2*y
  _2y.ModInv();             // 1/(2*y)
  _s.ModMulK1(&_p, &_2y);   // s = 3*x*x/(2*y)

  // rx = s^2 - 2*x
  _p.ModMulK1(&_s, &_s);
  Int _2x;
  _2x.ModAdd(&p1.x, &p1.x);
  r.x.ModSub(&_p, &_2x);
...
Point Secp256K1::Negation(Point &p)
{
  ...
  Point result(p);
  result.Reduce(); // Ensure normalized
  result.y.ModNeg();
  return result;
}
...
Point Secp256K1::ScalarMultiplication(Point &P, Int *scalar, bool debug)
{
  ...
  for (int i = bits - 1; i >= 0; i--)
  {
    ...
    if (scalar->GetBit(i))
    {
      ...
      {
        if (debug)
          printf("First 1 bit, setting to P\n");
        result = addend;
      }
      else
      {
